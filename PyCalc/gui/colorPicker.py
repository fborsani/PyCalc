...
class QColorSpinEdit(QtWidgets.QWidget):
    ...
    def __init__(self, parent=None):
        ...
        self.colorChanged.connect(self.syncSliders)
...
class QColorEdit(QtWidgets.QDialog):
    colorChanged = QtCore.pyqtSignal('QColor')
    ...
    def _syncWidgets(self, c):
        if self.sender() is self.lineEdit:
            if isinstance(self.spinColEdit, QColorSpinEdit):
                self.spinColEdit.syncSliders(c)
    ...
    def eventFilter(self, source, event):
        if source is self.spinColEdit and event.type() in (
                QtCore.QEvent.Close, QtCore.QEvent.Hide, QtCore.QEvent.HideToParent):
